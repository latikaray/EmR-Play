...
const VerifyOTPPage = () => {
  ...
  // Phase A: this page is parent-only. Child accounts no longer use OTP.
  // If someone lands here with role=child in the URL, redirect them away.
  if (role && role !== 'parent') {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        // PHASE A FIX: auth.uid() is immediately available after verifyOtp
        // resolves with a user — no setTimeout needed. The session is fully
        // committed synchronously before this promise resolves.
        const { error: roleError } = await supabase.rpc('claim_user_role', {
          p_role: 'parent',
        });

        if (roleError) {
          // PHASE A FIX: do NOT navigate on RPC failure. Show a recoverable
          // error so the user can retry without getting stuck in a redirect loop.
          console.error('Error creating user role:', roleError);
          toast.error(
            "Account verified but role setup failed. Please try signing in — if the problem persists, contact support."
          );
          // Sign the user out so they start fresh rather than entering a broken state.
          await supabase.auth.signOut();
          return;
        }

        // Create profile in the parent_profiles table.
        ...
        if (profileError) {
          ...
          toast.error("Account verified but profile creation failed. Please try signing in.");
          // Profile failure is non-fatal — user can still log in and profile
          // will be created on next load. Navigate anyway.
        } else {
